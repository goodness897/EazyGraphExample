...
import com.compet.eazygraphexample.models.BarModel;
import com.compet.eazygraphexample.models.BaseModel;
import com.compet.eazygraphexample.models.StackedBarModel;
...
/**
 * A rather simple type of a bar chart, where all the bars have the same height and their inner bars heights are
...
public class StackedBarChart extends BaseBarChart {
    ...
    public StackedBarChart(Context context, AttributeSet attrs) {
        super(context, attrs);
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.StackedBarChart, 0, 0);
        ...
        try {
            ...
            mSeparatorWidth = a.getDimension(R.styleable.StackedBarChart_egSeparatorWidth,
                                             Utils.dpToPx(DEF_SEPARATOR_WIDTH));
        ...
        for (StackedBarModel model : mData) {
            ...
            for (int index = 0; index < model.getBars().size(); index++) {
                ...
                if (mShowValues && barModel.isShowValue()) {
                    _Canvas.drawText(String.valueOf(barModel.getValue()),
                                     bounds.centerX(),

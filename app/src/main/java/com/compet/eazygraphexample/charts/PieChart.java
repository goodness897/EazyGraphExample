...
import com.compet.eazygraphexample.communication.IOnItemFocusChangedListener;
import com.compet.eazygraphexample.models.PieModel;
...
/**
 * A customizable PieChart which can be rotated to select a Pie Slice. It is possible to show a normal Pie Chart or a
 * Doughnut Pie Chart depending on the InnerPadding attributes.
 */
public class PieChart extends BaseChart {
    ...
    public PieChart(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            mValueTextSize = a.getDimension(R.styleable.PieChart_egValueTextSize,
                                            Utils.dpToPx(DEF_VALUE_TEXT_SIZE));
    ...
    /**
     * Checks if the AutoCenter is activated or not. AutoCenter is only available for API Level 11 and higher.
     *
    ...
    /**
     * Sets the AutoCenter property. AutoCenter is only available for API Level 11 and higher.
     *
    ...
    /**
     * Sets the property which indicates whether the currently selected PieSlice's value should be drawn in the center
     * or not.
     *
    ...
    /**
     * Sets the indication whether a custom inner value should be drawn or not. If it's true, no value of the currently
     * selected PieSlcie is shown in the center of the PieChart.
     *
    ...
    /**
     * Set the currently selected item. Calling this function will set the current selection and rotate the pie to bring
     * it into view.
     *
    ...
    /**
     * Set the current item by index. Optionally, scroll the current item into view. This version is for internal
     * use--the scrollIntoView option is always true for external callers.
     *
     * @param currentItem The index of the current item.
     * @param scrollIntoView True if the pie should rotate until the current item is centered. False otherwise. If this
     * parameter is false, the pie rotation will not change.
     */
    ...
    /**
     * Adds a new Pie Slice to the PieChart. After inserting and calculation of the highlighting color a complete
     * recalculation is initiated.
     *
    ...
    private void highlightSlice(PieModel _Slice) {
        ...
        _Slice.setHighlightedColor(Color.argb(0xff,
    ...
    /**
     * Kicks off an animation that will result in the pointer being centered in the pie slice of the currently selected
     * item.
     */
    private void centerOnCurrentItem() {
        if (!mPieData.isEmpty()) {
            ...
            if (mOpenClockwise) {
                targetAngle = (mIndicatorAngle - current.getStartAngle())
                              - ((current.getEndAngle() - current.getStartAngle()) / 2);
    ...
    protected void onGraphOverlayDraw(Canvas _Canvas) {
        ...
        if (!mPieData.isEmpty() && mDrawValueInPie) {
            ...
            if (!mUseCustomInnerValue) {
                mInnerValueString = Utils.getFloatString(model.getValue(),
                                                                                            mShowDecimal);
            ...
            _Canvas.drawText(mInnerValueString,
    ...
    protected void onLegendDraw(Canvas _Canvas) {
        ...
        float height =
                     mMaxFontHeight =
                                    Utils.calculateMaxTextHeight(mLegendPaint, null);
        ...
        if (!mPieData.isEmpty()) {
            ...
            _Canvas.drawText(mEmptyDataText,
                             (mLegendWidth / 2) - (mTextBounds.width() / 2),
                             mIndicatorSize * 2 + mIndicatorBottomMargin + mIndicatorTopMargin + height,
                             mLegendPaint);
        }
    ...
    protected void onGraphSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mInnerBounds = new RectF(mGraphBounds.centerX() - mCalculatedInnerPadding
        ...
        mInnerOutlineBounds = new RectF(mGraphBounds.centerX() - mCalculatedInnerPadding,
    ...
    private class GestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            float scrollTheta =
                              Utils.vectorToScalarScroll(distanceX,
                                                                                            distanceY,
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            ...
            float scrollTheta =
                              Utils.vectorToScalarScroll(velocityX,
                                                                                            velocityY,
                                                                                            e2.getX() - getGraphBounds().centerX(),
                                                                                            e2.getY() - getGraphBounds().centerY());
            mScroller.fling(0,
